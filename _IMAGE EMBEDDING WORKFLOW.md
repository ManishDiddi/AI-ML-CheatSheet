...
| AI Engineering — Advanced AI Agents track (new notes, standing rule) | **lecture-PDF figures** | ✅ AI Agents — Foundations 4 (self-made) 2026-08-03 · **Agentic Workflows 8** + **Model Context Protocol 8** (2026-08-09) · **Fine-Tuning LLMs 10** + **Distributed Training for LLMs 3** (2026-08-15 — 6 extracted from the handwritten lecture PDFs via the `pdfimages` + smask-composite route, 7 self-made matplotlib) · **Model Quantization 8** (2026-08-20 — 1 extracted clean from under the ink on p7 of `Advanced_fine_tuning___Model_Quantization_.pdf` via `pdfimages -f 7`, 7 self-made matplotlib: float bit-layouts, absmax-vs-affine mapping, NF4-vs-INT4 quantile level placement, activation-outlier heatmap + LLM.int8 split, 70B memory ladder, PTQ/QAT/QLoRA pipelines, per-tensor/channel/group granularity) |
