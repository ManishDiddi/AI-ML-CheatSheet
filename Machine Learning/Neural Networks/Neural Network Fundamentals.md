...
**Where it fits:** The foundation of **deep learning** — every CNN, RNN, and Transformer is this plus structure. It's [logistic/softmax regression](../Supervised%20ML/Logistic%20Regression.md) stacked and made non-linear. Optimizers/initialization get their own note: [[Weight Initialization & Optimizers]].
...
- **Vanishing gradients** — sigmoid/tanh in deep stacks (§7). Fix: **ReLU/variants**, residual connections, batch norm, and careful **initialization** (see [[Weight Initialization & Optimizers]]).
...
- **Overfitting** — lots of parameters memorize noise; combat with **dropout, batch norm, early stopping, weight decay (L2)**, and more data.
