...
**Where it fits:** The measurement layer of AI engineering — how you choose a model, catch regressions, and prove an LLM app is good enough to ship. Builds directly on [LLM](LLM.md) (decoding, hallucination) and [RNN · LSTM · Transformers](../Machine%20Learning/RNN%20%C2%B7%20LSTM%20%C2%B7%20Transformers.md) (tokenization, embeddings for BERTScore).
...
```
Step 3 is where **[Prompt Engineering](Prompt%20Engineering.md)** pays off — the rubric prompt *is* the metric, so a clear scale, few-shot examples, and chain-of-thought make the judge sharper (that's the natural next topic after this lecture).
