...
**And the question after training ends:** none of the above applies to *serving* the model — there are no gradients, no optimizer states, and nothing for ZeRO to shard. Inference reuses TP (now a **latency** lever, since decode is bandwidth-bound) and PP (still a fit-the-model lever), and adds an entirely different set of constraints — KV cache, continuous batching, TTFT/ITL. See [Serving LLMs at Scale](Serving%20LLMs%20at%20Scale.md).
